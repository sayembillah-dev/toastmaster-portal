...
function createTextTexture(gl, text, subtitle, font = 'bold 30px monospace', color = 'black') {
  const canvas = document.createElement('canvas');
  const context = canvas.getContext('2d');
  const fontSize = getFontSize(font);
  const lineHeight = fontSize * 1.3;
  const familyMatch = font.match(/\d+px\s+(.+)$/);
  const family = familyMatch ? familyMatch[1] : 'monospace';
  const boldFont = `bold ${fontSize}px ${family}`;
  const regularFont = `${Math.round(fontSize * 0.85)}px ${family}`;

  context.font = boldFont;
  const titleWidth = Math.ceil(context.measureText(text).width);
  context.font = regularFont;
  const subWidth = subtitle ? Math.ceil(context.measureText(subtitle).width) : 0;

  const canvasWidth = Math.max(titleWidth, subWidth) + 24;
  const canvasHeight = subtitle ? Math.ceil(lineHeight * 2 + 8) : Math.ceil(lineHeight + 8);
  ...
  if (subtitle) {
    ...
    context.fillText(text, canvas.width / 2, lineHeight);
    context.font = regularFont;
    context.globalAlpha = 0.7;
    context.fillText(subtitle, canvas.width / 2, lineHeight + lineHeight * 0.95);
...
class Media {
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, subtitle, viewport, bend, textColor, borderRadius = 0, font }) {
    this.extra = 0;
  ...
  createTitle() {
    this.title = new Title({ gl: this.gl, plane: this.plane, renderer: this.renderer, text: this.text, subtitle: this.subtitle, textColor: this.textColor, font: this.font });
  }

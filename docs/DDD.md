...
**Identity**: A member is uniquely identified globally by their Phone Number (the official Toastmasters PN is optional metadata).
...
**Invitation Delivery**: Unique, role-scoped links are generated by the system and manually shared (e.g., via WhatsApp) by the inviting officer.
**Super Admin Override**: The Super Admin can bypass the strict hierarchy to invite anyone (e.g., directly inviting a standard Member). The system uses **Context-Aware Invites**, requiring the Super Admin to select both the target Role and the target Context (e.g., Club XYZ) to bake into the generated magic link.
...
-   Member Conversion Service: Converts a guest lead into an active Club Member entity (granting them a persistent phone-based identity) upon fee payment and executive approval.
...
### 6.1 Identity & Member Profile Domain
- **Multi-Tenant Identity Architecture**: Instead of a single complex multi-tenant User record, the system creates separate `Member` records for each club context. These records are linked relationally across the platform using the global Toastmasters Member ID as a foreign key. This ensures clean separation of roles and statuses per club while allowing cross-club identity mapping.
